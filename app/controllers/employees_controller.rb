class EmployeesController < ApplicationController
  ...
  def index
    ...
    if params[:format] == 'csv'
      @employees = @employees.where("(id >= #{params[:from].present? ? params[:from] : 1}) AND (id <= #{params[:to].present? ? params[:to] : Employee.last.id})")
      generate_csv_headers("negotiations-#{Time.now.strftime("%Y%m%d")}")
    else
      @employees = @employees.paginate(:per_page => 5, :page => params[:page])
    end
